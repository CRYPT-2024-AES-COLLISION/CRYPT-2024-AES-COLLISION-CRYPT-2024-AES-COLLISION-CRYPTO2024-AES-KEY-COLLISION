...
key_xor = [[XOR(),XOR(),XOR(),XOR()] for _ in range(round+1)]

enc_prob = [[Node(),Node(),Node(),Node()] for _ in range(round+2)]
key_prob = [[Node(),Node(),Node(),Node()] for _ in range(round+1)]
...
for i in range(round+1):
    for j in range(4):
        ...
        dic["key_prob"][i][j].dir[3] = ("enc_xor", i, j)
...
# 最後のラウンドの下方向
for j in range(4):
    dic["key_prob"][round][j].dir[0] = ("", -100, -100)

# 全ラウンドの鍵スケジュールの一番右
for i in range(round+1):
    dic["key_prob"][i][3].dir[2] = ("key_xor", i, 0)

# 最後のラウンドの一番右
dic["key_prob"][round][3].dir[2] = ("", -100, -100)
...
for i in range(round+1):
    for j in range(4):
        ...
        dic["enc_xor"][i][j].dir[3] = ("key_prob", i, j)
...
for i in range(round+1):
    dic["key_xor"][i][0].dir[2] = ("key_prob", i, 3)
...
key_cnt = 0
enc_cnt = 1
with open("log_done/AES128_relkey_collision/AES128_relkey_collision_double_R3_DCP512.txt", "r") as f:
    for line in f:
        if "dp" in line:
            if "key" in line:
                six = line.count("□■")
                seven = line.count("■■")
                dic["key_prob"][key_cnt][3].prob = six*6+seven*7
                key_cnt += 1
            if "enc" in line:
                line = line.strip("\n")
...
inbound_node = [("enc_prob", 0, j) for j in range(4)] + [("enc_prob", 1, j) for j in range(3,-1,-1)]
inbound_idx = dict()

for d in [("enc_prob", 0, j) for j in range(4)]:
    inbound_idx[d] = 0
for i, d in enumerate([("enc_prob", 1, j) for j in range(3,-1,-1)]):
    inbound_idx[d] = i+1
...
#     for dd in d:
#         print(format(dd.group, "08b").replace("1","■").replace("0","□"), end=", ")
...
for d in dic["enc_mc"]:
    for dd in d[0].group:
        print(format(dd, "08b").replace("1","■").replace("0","□"), end=", ")
    print()

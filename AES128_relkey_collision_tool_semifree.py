...
key_prob = [[Node(),Node(),Node(),Node()] for _ in range(round+2)]
...
# 最後のラウンドの一番右
dic["key_prob"][round+1][3].dir[2] = ("", -100, -100)
...
key_cnt = 0
enc_cnt = 1
with open("log_done/AES128_relkey_collision/AES128_relkey_collision_semifree_R5_DCP256.txt", "r") as f:
    for line in f:
        if "dp" in line:
            if "key" in line:
                six = line.count("□■")
...
inbound_node = [("enc_prob", 2, j) for j in range(3,-1,-1)] + [("enc_prob", 3, j) for j in range(3, -1, -1)]
inbound_idx = dict()

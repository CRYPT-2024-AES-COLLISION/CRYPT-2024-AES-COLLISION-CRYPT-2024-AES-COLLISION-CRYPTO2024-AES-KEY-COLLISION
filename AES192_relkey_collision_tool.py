...
key_max_round = [-100, 2, 2, 3, 4, 4, 5, 6, 6, 7, 8, 8, 9]

enc_xor = [[XOR(),XOR(),XOR(),XOR()] for _ in range(round+1)]
key_xor = [[XOR(),XOR(),XOR(),XOR(), XOR(),XOR()] for _ in range(key_max_round[round]-1)]

enc_prob = [[Node(),Node(),Node(),Node()] for _ in range(round+2)]
key_prob = [[Node(),Node(),Node(),Node(), Node(),Node()] for _ in range((key_max_round[round]))]
...
#　最後のラウンドの下方向
for j in range(6):
    dic["key_prob"][key_max_round[round]-1][j].dir[0] = ("",-100,-100)

# 全ラウンドの鍵スケジュールの一番右
for i in range(key_max_round[round]):
    dic["key_prob"][i][5].dir[2] = ("key_xor",i,0)

# 最後のラウンドの一番右
dic["key_prob"][key_max_round[round]-1][5].dir[2] = ("",-100,-100)

# 必要のない鍵部分
print(4*(round+1), 6*(key_max_round[round]))
for i in range(4*(round+1), 6*(key_max_round[round])):
    print(i//6, i%6)
    dic["key_prob"][i//6][i%6].dir = [("",-100,-100),("",-100,100),("",-100,100),("",-100,100)]
...
for i in range(round+1):
    for j in range(4):
        ...
        dic["enc_xor"][i][j].dir[3] = ("key_prob",(i*4+j)//6,(i*4+j)%6)
        dic["enc_xor"][i][j].active = 0b0100

# ----- key_xor -----
for i in range(key_max_round[round]-1):
    for j in range(6):
...
for i in range(key_max_round[round]-1):
    dic["key_xor"][i][0].dir[2] = ("key_prob", i,5)


# ---------------------------------------------- ファイル読み込み ----------------------------------------------
key_cnt = 0
enc_cnt = 1
with open("log_done/AES192_relkey_collision/AES192_relkey_collision_R5_DCP192.txt", "r") as f:
    for line in f:
        if "dp" in line:
            if "key" in line:
                six = line.count("□■")
                seven = line.count("■■")
                dic["key_prob"][key_cnt][5].prob = six*6+seven*7
                key_cnt += 1
            if "enc" in line:
                line = line.strip("\n")
...
# inbooundするノードを決め打ち
inbound_node = [("enc_prob",0,j) for j in range(4)] + [("enc_prob",1,j) for j in range(4)] + [("enc_prob",2,j) for j in range(2)]
inbound_idx = dict()

for d in [("enc_prob", 0, j) for j in range(4)]:
    inbound_idx[d] = 0
for i, d in enumerate([("enc_prob",1,j) for j in range(4)] + [("enc_prob",2,j) for j in range(2)]):
    inbound_idx[d] = i+1
...
for d in dic["enc_prob"]:
    for dd in d:
        print(format(dd.group, "010b").replace("1","■").replace("0","□"), end=", ")
...
for d in dic["enc_mc"]:
    for dd in d[0].group:
        print(format(dd, "010b").replace("1","■").replace("0","□"), end=", ")
    print()

...
key_cnt = 0
enc_cnt = 1
with open("log_done/AES192_relkey_collision/AES192_relkey_collision_semifree_R7_DCP291.txt", "r") as f:
    for line in f:
        if "dp" in line:
            if "key" in line:
                six = line.count("□■")
...
# inbooundするノードを決め打ち
inbound_node = [("enc_prob",3,j) for j in range(4)] + [("enc_prob",4,j) for j in range(4)] + [("enc_prob",5,j) for j in range(2)]
inbound_idx = dict()

...
key_cnt = 0
enc_cnt = 1
with open("log_done/AES256_relkey_collision/AES256_relkey_col_semifree_R9_DCP193.txt", "r") as f:
    for line in f:
        if "dp" in line:
            if "key" in line:
                if key_cnt % 2 == 0:
...
# inbooundするノードを決め打ち
inbound_node = [("enc_prob",7,j) for j in range(3,-1,-1)] + [("enc_prob",8,j) for j in range(3,-1,-1)] + [("enc_prob",9,j) for j in range(3,-1,-1)]
inbound_idx = dict()
